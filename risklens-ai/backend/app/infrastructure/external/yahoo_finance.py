...
class YahooFinanceClient:
    ...
    def get_current_prices(symbols: List[str]) -> Dict[str, dict]:
        ...
        Returns:
            Dict mapping symbol -> {price, previous_close, day_change_pct, volume}
        """
        results = {}
        try:
    ...
    def get_historical_returns(
        ...
        Returns:
            Dict mapping symbol -> list of daily returns (most recent first)
        """
        results = {}
        end_date = datetime.now()
        start_date = end_date - timedelta(days=int(days * 1.5))  # Extra days for weekends
    ...
    def get_volatility(symbol: str, current_days: int = 30, previous_days: int = 90) -> dict:
        """Calculate 30-day realized volatility and compare to previous quarter.

        Args:
            symbol: Ticker symbol
            current_days: Window for current volatility
            previous_days: Window for previous quarter volatility

        Returns:
            Dict with volatility_30d, volatility_prev_quarter, change_pct
        """
        try:
            import numpy as np
            ticker = yf.Ticker(symbol)
            ...
            # Current 30-day volatility (annualized)
